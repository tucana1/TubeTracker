...
# benchmark/labels/m2_v1.json. A fixed random sample of 30 isolated grains is labelled (the rest
# start excluded as "not sampled"). Close this window (or press Ctrl-C) when you are done.
cd "$(dirname "$0")" || exit 1
...
.venv/bin/python -m sparsetrack bench "$CACHE" --labels "$LABELS" --annotator investigator --port 8766 --sample 30
